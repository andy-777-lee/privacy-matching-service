...
```
[가입]        휴대폰 번호 입력 → /api/send-otp (SMS 발송)
              → 인증번호 입력 → /api/verify-otp (커스텀 토큰 발급)
              → 가입 폼에서 숫자 4자리 비밀번호 설정 → /api/set-pin
[로그인]      휴대폰 번호 + 비밀번호 4자리 → /api/login-pin → 로그인 (SMS 없음)
[비밀번호 분실] "처음이신가요? 비밀번호를 잊으셨나요?" → 인증번호로 로그인
              → 로그인 후 비밀번호 재설정 안내
[알림]        프로필 요청/수락/거절/매칭완료 시 → /api/notify → 상대방에게 SMS
```

평상시 로그인에는 SMS가 발송되지 않으므로, 문자 비용은 **가입 시 1회**와
비밀번호 분실 시에만 발생합니다.

**비밀번호 보안**

- 해시는 `user_auth/{uid}`에 저장되며, 클라이언트는 읽을 수도 쓸 수도 없습니다
  (firestore.rules의 기본 거부 규칙이 적용됨). Admin SDK만 접근합니다.
- scrypt + 사용자별 랜덤 salt로 해싱하고, 비교는 타이밍 공격에 안전한 방식을 씁니다.
- 4자리는 10,000가지뿐이라 **연속 5회 실패 시 10분 잠금**이 걸립니다.
  잠긴 계정은 인증번호로 로그인하면 즉시 풀립니다.
- `0000`, `1111`, `1234`, `4321` 같은 단순한 번호는 설정 단계에서 거부됩니다.
